...
PCP);
    ...
    public function test_compiler_handles_default_slot_end_to_end(): void
    {
        $componentsDir = $this->tmpDir . DIRECTORY_SEPARATOR . 'components_default';
        $cacheDir = $this->tmpDir . DIRECTORY_SEPARATOR . 'cache_default';
        ...
        file_put_contents($appComponentsDir . DIRECTORY_SEPARATOR . 'Panel.pcp', <<<'PCP'
namespace App\Components;
...
PCP);
        ...
        file_put_contents($appComponentsDir . DIRECTORY_SEPARATOR . 'PageDefault.pcp', <<<'PCP'
namespace App\Components;

use PCP\Component;
use PCP\Runtime\Node;

final class PageDefault extends Component
{
    public function render(): Node
    {
        return (
...
PCP);
        ...
        $panelSource = $resolver->requireComponent('App\\Components\\Panel');
        $panelCompiled = $compiler->compileIfNeeded('App\\Components\\Panel', $panelSource);
        require $panelCompiled;

        $pageSource = $resolver->requireComponent('App\\Components\\PageDefault');
        $pageCompiled = $compiler->compileIfNeeded('App\\Components\\PageDefault', $pageSource);
        require $pageCompiled;

        $page = new \App\Components\PageDefault();

        $html = $page->render()->toHtml();

        self::assertSame(
            '<section class="panel"><p>Contenido por defecto</p></section>',
            $this->normalizeHtml($html),
        );
    }

    public function test_compiler_handles_default_slot_fallback_end_to_end(): void
    {
        $componentsDir = $this->tmpDir . DIRECTORY_SEPARATOR . 'components_default_fallback';
        $cacheDir = $this->tmpDir . DIRECTORY_SEPARATOR . 'cache_default_fallback';
        ...
        file_put_contents($appComponentsDir . DIRECTORY_SEPARATOR . 'PanelFallback.pcp', <<<'PCP'
namespace App\Components;

use PCP\Component;
use PCP\Runtime\Node;

final class PanelFallback extends Component
{
    public function render(): Node
    {
        return (
            <section class="panel">
                <Slot:default><p>Fallback default</p></Slot:default>
            </section>
        );
    }
}
PCP);
        ...
        file_put_contents($appComponentsDir . DIRECTORY_SEPARATOR . 'PageDefaultFallback.pcp', <<<'PCP'
namespace App\Components;

use PCP\Component;
use PCP\Runtime\Node;

final class PageDefaultFallback extends Component
{
    public function render(): Node
    {
        return (
...
PCP);
        ...
        $panelSource = $resolver->requireComponent('App\\Components\\PanelFallback');
        $panelCompiled = $compiler->compileIfNeeded('App\\Components\\PanelFallback', $panelSource);
        require $panelCompiled;

        $pageSource = $resolver->requireComponent('App\\Components\\PageDefaultFallback');
        $pageCompiled = $compiler->compileIfNeeded('App\\Components\\PageDefaultFallback', $pageSource);
        require $pageCompiled;

        $page = new \App\Components\PageDefaultFallback();

        $html = $page->render()->toHtml();

        self::assertSame(
            '<section class="panel"><p>Fallback default</p></section>',
            $this->normalizeHtml($html),
        );
    }
